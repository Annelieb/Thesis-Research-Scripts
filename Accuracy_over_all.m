%% Evaluate online; Subject accuracy all sessions (training + feedback) cut like in online

% Initial parameters

clear all

Subjects = [6 7]; % Subject that needs to be evaluated
    ...
    for TF = 1:2
        ...
        for trll = 1:length(TRL)
            ...
            for HH = 1:noftrials % repeat for all trials - leave one out
                ...
                CA{trll}(S,train(end)) = corr2(recenv,Att_recut{train(end)}'); % Attended
                CUA{trll}(S,train(end)) = corr2(recenv,Unatt_recut{train(end)}'); % Unattended
...
%             end
                ...
                difference_all{trll}(S,:) = CA{trll}(S,:) - CUA{trll}(S,:);
                Accuracy_all(S,trll) = sum(difference_all{trll}(S,:)>0)/noftrials*100
