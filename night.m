...
%HP = [6,7, 8, 9, 10];
%NSR = 20;
NewSamplingRate = [10 20 30 40] % 50 80]; % 200 500]; % New sampling rate; 200 and 500 take to much computing time
...
cd('C:\Users\Annelies\Documents\Studie\Thesis\script\All_data\New bp filter\audio HL = []\Investigate seperate lags\Investigate NSR')
...
for TRL = 2:6
    ...
    for S = 1:nofsubj
        ...
        % Accuracy seperate time lags
        noftrials = size(Attended{S},1);
        train = (1:noftrials);
         start = floor(lag(1)/1e3*NSR);
         fin = ceil(lag(2)/1e3*NSR);
        noflag = length(start:fin);
        for l = 1:noflag
        for HH = 1:noftrials % repeat for all trials - leave one out
            trials2 = train(1:end-1);
        
            RXX = squeeze(mean(covar.Rxx_all(S,trials2,(l-1)*24 +1 : l*24,(l-1)*24 +1 : l*24),2)); % plain averaging of cov matrices
            RXY_ATT = squeeze(mean(covar.Rxy_at_all(S,trials2,(l-1)*24 +1 : l*24),2));
        ...
        Accuracy_ind_lags(TRL,isr, nsr,S,l) = (sum(CA-CUA> 0)/(noftrials))*100 % Final accuracy on all trials
        CA = []; CUA = [];
        end
...
%         end
        ...
        cd('C:\Users\Annelies\Documents\Studie\Thesis\script\All_data\New bp filter\audio HL = []\Investigate seperate lags\Investigate NSR')
        save(['Result_NSR4_TRL2to6.mat'], 'Subject_Accuracy', 'Accuracy_ind_lags')
...
for isr = 8:8
    ...
    intermediateSampleRate = intSamp(isr)
for TRL = 4:6 % Loop for trial length
...
cd('C:\Users\Annelies\Documents\Studie\Thesis\script\All_data\New bp filter\audio HL = []\Investigate lags')

%save(['TRL' num2str(TRL) '_ISR' num2str(isr) '_NSR' num2str(nsr) '.mat'], 'EE', 'covar', 'Attended', 'Unattended', 'sessionlength') % , '-v7.3') Vanaf NSR > 50 nodig
%save(['LP' num2str(LP) '_HP' num2str(HP) '_TRL' num2str(TRL) '_ISR' num2str(isr) '_NSR' num2str(nsr) '.mat'], 'EE', 'covar', 'Attended', 'Unattended', 'sessionlength') % , '-v7.3') Vanaf NSR > 50 nodig
save(['lag1' num2str(l1), '_lag2' num2str(l2) '_TRL' num2str(TRL) '_ISR' num2str(isr) '_NSR' num2str(nsr) '.mat'], 'EE', 'covar', 'Attended', 'Unattended', 'sessionlength') % , '-v7.3') Vanaf NSR > 50 nodig
...
cd('C:\Users\Annelies\Documents\Studie\Thesis\script\All_data\New bp filter\audio HL = []\Investigate lags')
Subject_Accuracy = [];
for l1 = 1:length(lag1)
    for l2 = 1:length(lag2)
        lag = [lag1(l1), lag2(l2)];
for nsr = 3:3
for isr = 8:8
for TRL = 1:6
    ...
    %load(['LP' num2str(LP) '_HP' num2str(HP) '_TRL' num2str(TRL) '_ISR' num2str(isr) '_NSR' num2str(nsr) '.mat'])
    load(['lag1' num2str(l1), '_lag2' num2str(l2) '_TRL' num2str(TRL) '_ISR' num2str(isr) '_NSR' num2str(nsr) '.mat'])
    ...
    for S = 1:nofsubj
        ...
        Subject_Accuracy(TRL,isr, nsr,S, l1, l2) = (sum(CA-CUA> 0)/(noftrials))*100 % Final accuracy on all trials
        CA = []; CUA = [];
...
%         end
        ...
        cd('C:\Users\Annelies\Documents\Studie\Thesis\script\All_data\New bp filter\audio HL = []\Investigate lags')
        save(['Result_lags.mat'], 'Subject_Accuracy')
